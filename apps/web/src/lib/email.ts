...
export async function sendHarvestReadyEmail(
  to: string,
  name: string,
  productName: string,
  orderNumber: string,
  ...
  await transporter.sendMail({
    from:    `"Vuna" <${process.env.SMTP_USER}>`,
    to,
    subject: `Your harvest is in — ${productName} is ready`,
    html: `
      <div style="font-family:Georgia,serif;max-width:480px;margin:0 auto;background:#FAFAF9;padding:32px;border-radius:12px;">
        <div style="text-align:center;margin-bottom:24px;">
          <div style="background:#7C2D12;color:#FEF3C7;font-size:22px;font-weight:900;
        ...
        <p style="color:#374151;font-size:15px;line-height:1.6;margin-bottom:16px;">
          Good news from the farm — the harvest you reserved is in!
          <strong>${productName}</strong> is ready, and your reservation
          (order #${orderNumber.slice(-8).toUpperCase()}) is now open for payment.
        </p>
        <p style="color:#374151;font-size:15px;line-height:1.6;margin-bottom:24px;">
          Pay the farmer directly by EFT and upload your proof of payment — the same trusted
          Vuna way. Your share is put aside for you.
        </p>
        <div style="text-align:center;margin-bottom:24px;">
          <a href="${payUrl}"
             style="background:#7C2D12;color:#FEF3C7;text-decoration:none;padding:14px 32px;
...
export async function sendHarvestFailedEmail(
  to: string,
  name: string,
  productName: string,
) {
  await transporter.sendMail({
    from:    `"Vuna" <${process.env.SMTP_USER}>`,
    to,
    subject: `Harvest update — ${productName}`,
    html: `
      <div style="font-family:Georgia,serif;max-width:480px;margin:0 auto;background:#FAFAF9;padding:32px;border-radius:12px;">
        <div style="text-align:center;margin-bottom:24px;">
          <div style="background:#7C2D12;color:#FEF3C7;font-size:22px;font-weight:900;
        ...
        <h2 style="font-size:18px;color:#1C0A00;margin-bottom:8px;">Hi ${name},</h2>
        <p style="color:#374151;font-size:15px;line-height:1.6;margin-bottom:16px;">
          Sad news from the farm: the harvest you reserved — <strong>${productName}</strong> —
          did not come in this season. Farming carries real risk, and this time the crop failed.
        </p>
        <p style="color:#374151;font-size:15px;line-height:1.6;margin-bottom:24px;">
          Your reservation has been cancelled and <strong>you owe nothing</strong> — on Vuna,
          money only moves on a harvest, never on a promise. We hope you will stand with this
          farmer again next season.
        </p>
        <hr style="border:none;border-top:1px solid #e5e7eb;margin:20px 0;" />
        <p style="color:#9CA3AF;font-size:11px;text-align:center;">
          Vuna &mdash; Where Local Is Celebrated And Cherished &mdash; Umzila-AfriRoute
